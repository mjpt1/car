...
import { listMyTransactions } from '../../lib/services/transactionService';
import DataTable from '../../components/admin/DataTable'; // Reusing admin DataTable
...
export default function MyTransactionsPage() {
  ...
  const fetchTransactions = useCallback(async (currentPage) => {
    ...
    setError('');
    try {
      const result = await listMyTransactions({ page: currentPage, limit: 10 });
      setData(result);
    } catch (err) {
  ...
  const columns = useMemo(() => [
    ...
    { Header: 'شناسه رزرو', accessor: 'booking_id', Cell: ({ row }) => row.booking_id || '-' },
  ...
  return (
    <div className="container mx-auto px-4 py-8 space-y-6">
      <header>
        <h1 className="text-3xl font-bold text-gray-800">تاریخچه تراکنش‌ها</h1>
        <p className="text-gray-600 mt-1">لیست تمام تراکنش‌های مالی شما.</p>
      </header>
